...
parser.add_argument('--src', default='G:/vai_for_pf', type=str, help='path for the original dataset')
parser.add_argument('--tar', default='G:/vai_for_pf/patches', type=str, help='path for saving the new dataset')
...
parser.add_argument('--patch_width', default=768, type=int, help='Width of the cropped image patch')
parser.add_argument('--patch_height', default=768, type=int, help='Height of the cropped image patch')
parser.add_argument('--overlap_area', default=256, type=int, help='Overlap area')  # Overlap = image size - stride
    ...
    for file_ in files:
        for extra in extras:
            ...
            if img_H > patch_H and img_W > patch_W:
                for x in range(0, img_W, patch_W-overlap):  # (start,stop[不含],step[≠0])
