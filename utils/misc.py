...
# Create unique output dir name based on non-default command line args 生成输出路径名
def make_exp_name(args, parser):
    exp_name = '{}-{}'.format(args.dataset[:4], args.arch[:])  # 实验名 数据集-网络
    dict_args = vars(args)
